...
beta_vals_prj %>%
  ...
  dplyr::left_join(as.data.frame(pheno_df), by = c("sample_name" = "geo_accession")) %>%
  dplyr::select(tumor_type) %>%
  unlist() -> tumor_type

plot_tsne(tsne_out_lst[[1]], colour = tumor_type)

# Plot multiple tSNE results
set.seed(456)
idx <- sample(x = seq(length(tsne_out_lst)), size = 5)
multiplot_tsne(tsne_out_lst[idx], pheno = tumor_type)
```
