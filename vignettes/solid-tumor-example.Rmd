...
```{r}
set.seed(125) # Sets seed for reproducibility
tsne_out <- Rtsne(beta_vals_transformed, perplexity = 5) # Run TSNE
col <- RColorBrewer::brewer.pal(3, name = "Dark2")
tsne_out$Y %>%
  as.data.frame() %>%
  ggplot(., aes(
    x = V1,
    y = V2,
    colour = df$`diagnosis:ch1`,
    label = df$geo_accession
  )) +
  geom_point() +
  labs(x = "", y = "", colour = "Diagnosis") +
  ggtitle("t-SNE") +
  theme_light() +
